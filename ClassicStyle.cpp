...
void ClassicStyle::drawComplexControl(ComplexControl cc,
                                      ...
                                      const QWidget *widget) const
{
    ...
    float angle = DIAL_MIN
                  + (DIAL_RANGE
                     * (float(dial->sliderValue - dial->minimum)
    ...
    for (int arc = 120; arc < 2880; arc += 240) {
        pen.setColor(c);
        p->setPen(pen);
        p->drawArc(indent, indent, width - 2 * indent, width - 2 * indent, shadowAngle + arc, 240);
        p->drawArc(indent, indent, width - 2 * indent, width - 2 * indent, shadowAngle - arc, 240);
        c = c.lighter(110);
    }
    ...
    for (int arc = 120; arc < 2880; arc += 240) {
        pen.setColor(c);
        p->setPen(pen);
        p->drawArc(scale / 2, scale / 2, width - scale, width - scale, shadowAngle + arc, 240);
        p->drawArc(scale / 2, scale / 2, width - scale, width - scale, shadowAngle - arc, 240);
        c = c.lighter(108);
    }
    ...
    p->setPen(pen);
    p->drawArc(scale / 2, scale / 2, width - scale, width - scale, -45 * 16, -90 * 16);
