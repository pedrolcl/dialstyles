/******************************************************************************
   ...
   Copyright (c) 2009-2024 Pedro López-Cabanillas <plcl@users.sf.net>
...
static void paintIndicatorCached(QPainter *painter,
...
#elif 1
    QLinearGradient dial_gradient(option->direction == Qt::LeftToRight ? r.topLeft() : r.topRight(),
                                  option->direction == Qt::LeftToRight ? r.bottomRight()
                                                                       : r.bottomLeft());
    //  QLinearGradient dial_gradient(r.topLeft(), r.bottomLeft());
...
void paintCachedDialBase(QPainter *painter, const QStyleOptionSlider *option)
{
    ...
    if (useCache) {
        uint state = uint(option->state)
                     & (QStyle::State_Enabled | QStyle::State_On | QStyle::State_MouseOver
                        | QStyle::State_KeyboardFocusChange | QStyle::State_HasFocus);
...
//  state &= ~(QStyle::State_HasFocus);
        ...
        pixmapName.sprintf("scp-qdb-%x-%x-%llx-%x",
                           state,
                           option->direction,
                           option->palette.cacheKey(),
                           d);
#else
        pixmapName = QString::asprintf("scp-qdb-%x-%x-%llx-%x",
                                       state,
                                       option->direction,
                                       option->palette.cacheKey(),
                                       d);
...
void paintCachedGrip(QPainter *painter, const QStyleOption *option, QPalette::ColorRole /*bgrole*/)
{
    ...
    if (/* option->state & (QStyle::State_HasFocus | QStyle::State_MouseOver) ||*/ option->rect
                .width()
            * option->rect.height()
        > 4096) {
        useCache = false;
...
#if QT_VERSION < QT_VERSION_CHECK(5, 5, 0)
        pixmapName.sprintf("scp-isg-%x-%x-%s-%x-%x",
                           state,
                           option->direction,
                           colorName.constData(),
                           option->rect.width(),
                           option->rect.height());
#else
        pixmapName = QString::asprintf("scp-isg-%x-%x-%s-%x-%x",
                                       state,
                                       option->direction,
                                       colorName.constData(),
                                       option->rect.width(),
...
void SkulptureStyle::drawComplexControl(ComplexControl cc,
                                        ...
                                        const QWidget *widget) const
{
    ...
    if ((option->state & QStyle::State_HasFocus)
        && (slider = qobject_cast<const QAbstractSlider *>(widget))) {
    ...
    qreal angle;
    int sliderPosition = option->upsideDown ? option->sliderPosition
                                            : (option->maximum - option->sliderPosition);
    ...
    paintCachedGrip(painter,
                    &opt,
                    option->state & QStyle::State_Enabled ? QPalette::Button : QPalette::Window);
}
