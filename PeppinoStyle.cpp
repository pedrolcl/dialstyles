...
inline void paintArc(QPainter *p, const QStyleOptionSlider *dial)
{
    ...
    QColor arcColor = (dial->state & QStyle::State_Enabled) ? pal.highlight().color()
                                                            : pal.mid().color();
...
inline void paintDot(QPainter *p, const QStyleOptionSlider *dial)
{
    ...
    QPalette pal = dial->palette;
    QPen dotPen((dial->state & QStyle::State_Enabled) ? Qt::red : pal.mid().color(),
                7,
                Qt::SolidLine,
                Qt::RoundCap,
                Qt::RoundJoin);
    ...
    if (dial->subControls & QStyle::SC_DialTickmarks) {
        ...
        for (int i = 0; i <= dot; i++) {
            p->drawArc(rectangle2, 225 * 16 - delta * i, 5);
        }
    }
}

void PeppinoStyle::drawComplexControl(ComplexControl cc,
